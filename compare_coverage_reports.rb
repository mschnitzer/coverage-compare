...
minitest_coverage['coverage'].each do |file, minitest_lines|
  minitest_covered = minitest_lines.select {|l| l && l > 0}.count
  minitest_uncovered = minitest_lines.select {|l| !l || l == 0}.count

  diff_results[file] = {
    rspec_covered: rspec_coverage_results[file][:covered],
    rspec_uncovered: rspec_coverage_results[file][:uncovered],
    rspec_covered_percentage: "%.2f" % (rspec_coverage_results[file][:covered].to_f / minitest_lines.count.to_f * 100),
    rspec_uncovered_percentage: "%.2f" % (rspec_coverage_results[file][:uncovered].to_f / minitest_lines.count.to_f * 100),
    minitest_covered: minitest_lines.select {|l| l && l > 0}.count,
    minitest_uncovered: minitest_lines.select {|l| !l || l == 0}.count,
    minitest_covered_percentage: "%.2f" % (minitest_covered.to_f / minitest_lines.count.to_f * 100),
    minitest_uncovered_percentage: "%.2f" % (minitest_uncovered.to_f / minitest_lines.count.to_f * 100),
  }
  ...
  has_uncovered_data = false

  rspec_coverage_results[file][:lines].each_with_index do |line, index|
    # if the line is not covered in the rspec test suite, do a look up if it was covered in minitest
    if (!line || line == 0) && minitest_lines[index] && minitest_lines[index] > 0
      has_uncovered_data = true
      break
    end
  end

  if has_uncovered_data
...
## Acceptance Criteria
  ...
  * Both tests cover the same scenarios"
    ...
    # to give the Trello API a little break
    sleep 1
    next
  end

  puts "Skipping file '#{file}' because all relevant lines were covered."
end
